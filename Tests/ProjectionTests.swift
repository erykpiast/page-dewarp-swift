...
// Tests for projectXY() — ported from projection.py
...
final class ProjectionTests: XCTestCase {
    ...
    // All subsequent elements (ycoords, xcoords) are appended but unused by projectXY.
    ...
    // Generated with: python3 -c "..." (see scripts in golden_output/)
    ...
    func testAlphaClamped() {
        ...
        let clamped = projectXY(xyCoords: xy, pvec: pvec)
        let expected = projectXY(xyCoords: xy, pvec: pvecUnclamped)
    ...
    func testBetaClamped() {
        ...
        let clamped = projectXY(xyCoords: xy, pvec: pvec)
        let expected = projectXY(xyCoords: xy, pvec: pvecExpected)
    ...
    func testZeroCubicProducesZeroZ() {
        ...
        let xy = [[0.0, 0.0]]
        let result = projectXY(xyCoords: xy, pvec: pvec)
    ...
    func testOutputCount() {
        ...
        let result = projectXY(xyCoords: xy, pvec: goldenPvec)
    ...
    func testSinglePoint() {
        let result = projectXY(xyCoords: [[0.0, 0.0]], pvec: goldenPvec)
        XCTAssertEqual(result.count, 1)
        XCTAssertEqual(result[0].count, 2)
    }
    ...
    func testGoldenOutput() {
        // Compares Swift projectXY output against Python reference (tolerance 1e-4).
        // Ported from projection.py:19-57; expected values generated by Python.
        let result = projectXY(xyCoords: goldenXY, pvec: goldenPvec)
